...
function downloadSingleEvent(id) {
    // Typtolerant: Aus dem Detailfenster kommt die ID als Zeichenkette.
...
window.openEventModal = function (id) {
    // Typtolerant: Aus dem Fenster selbst kommt die ID als Zeichenkette.
    const event = globalEventsData.find(e => String(e.id) === String(id));
    if (!event) return;

    // Fuer Funktionen ausserhalb dieser Datei hinterlegen (siehe events-archive.js).
    ...
    modalBody.innerHTML = `
        ...
        <div class="modal-aktionen">
            <button class="btn btn-secondary" onclick="downloadSingleEvent('${String(event.id).replace(/'/g, "\\'")}')">
            ...
            <button id="termin-bild-btn" class="btn btn-secondary" onclick="window.teileTerminAlsBild('${String(event.id).replace(/'/g, "\\'")}')">
